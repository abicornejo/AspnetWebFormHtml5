...
************************************************************************************************************/
    ...
    var arrParamsUpd = new Array();
    var myMinimizedContentPage;
	...
	*/	
	...
	function hideWindow() {
      if (sessionStorage.usserCarritoOpen == "true" && jQuery.trim(activeWindow.innerText) != "" && jQuery.trim(activeWindow.innerText) == "Carrito de Compras") 
      {
	        sessionStorage.usserCarritoOpen = "false";
	        sessionStorage.usserCarritoOT = "";
	        sessionStorage.usserCarritoProp = "";    
	    }
        else
        {
            if (sessionStorage.usserCarritoOpen == "true" && jQuery.trim(activeWindow.innerText) == "")
            {
                $.each($(activeWindow).children(), function (index, myDiv) {
                    if (myDiv.id.indexOf("top_bar") == 0) {
                        $.each($(myDiv).children(), function (index, myDivC) {
                            if (myDivC.className == "titleBar" && jQuery.trim(myDivC.innerText) != "" && jQuery.trim(myDivC.innerText) == "Carrito de Compras") {
                                sessionStorage.usserCarritoOpen = "false";
                                sessionStorage.usserCarritoOT = "";
                                sessionStorage.usserCarritoProp = "";
                            }
                            if (myDivC.className == "top_buttons") {
                                $.each($(myDivC).children(), function (index, myDivP) {
                                    if (myDivP.id.indexOf("minimizeImage") == 0 && sessionStorage.usserCarritoOpen == "true" && jQuery.trim($(myDivP).attr("data-title")) == "Carrito de Compras") {
                                        sessionStorage.usserCarritoOpen = "false";
                                        sessionStorage.usserCarritoOT = "";
                                        sessionStorage.usserCarritoProp = "";
                                    }
        ...
        opnenedWindows[$("#windowContent" + this.id.replace(/[^\d]/g, '')).attr("data-identName")] = undefined;
		switchElement(false,document.getElementById('dhtml_goodies_id' + this.id.replace(/[^\d]/g,'')));
		activeWindow.style.display = 'none';
		if (arrToUpdate[this.id.replace(/[^\d]/g, '')] != undefined) {
		    if (arrParamsUpd[this.id.replace(/[^\d]/g, '')] != undefined)
		        arrToUpdate[this.id.replace(/[^\d]/g, '')].updateForm(arrParamsUpd[this.id.replace(/[^\d]/g, '')]);
		    else
		        arrToUpdate[this.id.replace(/[^\d]/g, '')].updateForm();
		    arrToUpdate[this.id.replace(/[^\d]/g, '')] == undefined;
		}
		document.body.removeChild(activeWindow);
		return true;
}

function closeWindow(windowId) {
    $(document.getElementById('closeImage' + windowId)).click();
}

function resizeWindow(windowId, newWidth, newHeigth) {
    if (newWidth != undefined) {
        $(document.getElementById('windowContent' + windowId)).find("iframe").attr("width", newWidth);
        $($($(document.getElementById('windowContent' + windowId)).parent()).parent()).css("width", newWidth + 7);
    }
    if (newHeigth != undefined) {
        $(document.getElementById('windowContent' + windowId)).find("iframe").attr("heigth", newHeigth);
        $($($(document.getElementById('windowContent' + windowId)).parent()).parent()).css("heigth", newHeigth);
    }
}

function RenameWindow(windowId, Titulo) {
    var a = $(document.getElementById('dhtml_goodies_id' + windowId));
    $.each($(a[0]).children(), function (index, myDiv) {
        var valor = myDiv;
        if (myDiv.id.indexOf("top_bar") != -1) {

            if ($(myDiv).children() != undefined && $(myDiv).children().length > 0) {
                if ($($(myDiv).children()[2]) != undefined && $($(myDiv).children()[2]).children().length > 0) {

                    $.each($($(myDiv).children()[2]).children(), function (index, myDivInt) {
                        if (myDivInt.id.indexOf("minimizeImage") != -1) {
                            $(myDivInt).attr("data-title", Titulo.replace("ó", "&oacute;").replace("í", "&iacute;").replace("ú", "&uacute;").replace("é", "&eacute;").replace("á", "&aacute;"));
                        }
                    });
                }
                if ($($(myDiv).children()[3]) != undefined) {
                    $($(myDiv).children()[3])[0].innerText = Titulo;
                }
            }
...
function isClosedWindow() {
    ...
    if (window.location.hostname = "localhost") {
        window.location = redirect + "Default.aspx"; //"http://" + window.location.hostname + ":" + window.location.port + "/FIATubeHtml5/Default.aspx";
    } else {
    
        window.location = redirect + "Default.aspx";
    }
}
function minimizeWindowCarrito(windowId) {
    var inputObj = document.getElementById('minimizeImage' + windowId);
   
    var numericID = inputObj.id.replace(/[^0-9]/g, '');
    switchElement(false, document.getElementById('dhtml_goodies_id' + numericID));
    var state;
    if (inputObj.src.indexOf('minimize') >= 0) {
        myMinimizedContentPage.addMinimizedItem(numericID, $(inputObj).attr('data-title'), $(inputObj).attr('data-key'));
        activeWindow.style.display = 'none';
...
function maximizeWindow(windowId) {
    $(document.getElementById('minimizeImage' + windowId)).click();
}
			...
			if(divs[no].className=='dhtmlgoodies_window'){	
				...
				for(var no2=0;no2<subImages.length;no2++){
					if(subImages[no2].className=='resizeImage'){
						subImages[no2].style.display = 'none';
...
function createNewWindow(width, height, left, top, title, key, isNotClosable) {    
		...
		if (isNotClosable == true) {
		    var img = document.createElement('IMG');
		    img.src = '../Images/vacio.png';
		    $(img).attr('style', 'padding-top: 3px;;');
	...
	function customFunctionCreateWindow(urlSource, width, height, left, top, title, isNotClosable, identifier) 
    {
        var divId;
        var windowId;
        ...
        if (title == "Video") {
            $("#divModal").empty();
            $("#divModal").dialog({ modal: true, width: 'auto', height: 'auto', close: function () { $(this).empty(); $(this).dialog("close"); }  });
            $("#divModal").append("<iframe src='" + urlSource + windowId + "' width='" + (width - 10) + "px' height='" + height + "px' />");
            $("#divModal").dialog('open');
            return 0;   
        }

        if (identifier != undefined && isRegisterWindow(identifier) == false) {
            divId = createNewWindow(width, height, left, top, title, urlSource.substring(0, urlSource.indexOf(".")), isNotClosable);
            opnenedWindows[identifier] = divId;
        }
        else if (identifier != undefined) 
            divId = opnenedWindows[identifier];
        else
            divId = createNewWindow(width, height, left, top, title, urlSource.substring(0, urlSource.indexOf(".")), isNotClosable);

        
        if (urlSource.toString().indexOf('?') >= 0)
            windowId = "&windowId=" + divId + "&XYZ=" + Math.floor(Math.random() * 9999);
        else
            windowId = "?windowId=" + divId + "&XYZ=" + Math.floor(Math.random() * 9999);

        document.getElementById('windowContent' + divId).innerHTML = "<iframe src='" + urlSource + windowId + "' width='" + (width - 10) + "px' height='" + height + "px' />";
        $("#windowContent" + divId).attr("data-identName", identifier);

        return divId;
    }

    function customFunctionCreateWindowUpdatable(urlSource, width, height, left, top, title, divToUpdate, params, identifier) {
        var divId;
        var windowId;

        if (identifier != undefined && isRegisterWindow(identifier) == false) {
            divId = createNewWindow(width, height, left, top, title, urlSource.substring(0, urlSource.indexOf(".")));
            opnenedWindows[identifier] = divId;
        }
        else if (identifier != undefined)
            divId = opnenedWindows[identifier];
        ...
        if (urlSource.toString().indexOf('?') >= 0)
            ...
            windowId = "?windowId=" + divId + "&XYZ=" + Math.floor(Math.random() * 9999);
        arrToUpdate[divId] = divToUpdate;
        if (params != undefined)
            arrParamsUpd[divId] = params;
        document.getElementById('windowContent' + divId).innerHTML = "<iframe src='" + urlSource + windowId + "' width='" + (width - 10) + "px' height='" + height + "px' />";
        return divId;
    }

    function customFunctionCreateWindowNotClosable(urlSource, width, height, left, top, title, divToUpdate, identifier) {
        var divId;
        var windowId;
	...
	function setMinimizedContentPage(minimizedContentPage) {
	    this.myMinimizedContentPage = minimizedContentPage;
    }

    /*Si la ventana no existe dentro del registro la registra y  manda falso*/
    function isRegisterWindow(identifier) {
        var resultado;
        if (opnenedWindows == undefined)
            opnenedWindows = new Array();

        if (opnenedWindows[identifier.toString()] != undefined) {
            resultado = true;
        }
        else {
            resultado = false;
