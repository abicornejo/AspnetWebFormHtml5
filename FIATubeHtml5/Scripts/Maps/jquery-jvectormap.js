...
(function ($) {
    ...
    VectorCanvas.prototype = {
        ...
        createLine: function (config, x1, y1, x2, y2) {
            var node;
            if (this.mode == 'svg') {
                node = this.createSvgNode('line');
                node.setAttribute('x1', x1 + 190);
                node.setAttribute('y1', y1);
                node.setAttribute('x2', x2 + 190);
                node.setAttribute('y2', y2);
                node.setAttribute('fill', config.fill);
                node.setAttribute('stroke', config.stroke);
                node.setAttribute('style', 'stroke:green;stroke-width:2;display:none;');
                node.setPosition = function (point) {
                    node.setAttribute('x1', point.x);
                    node.setAttribute('y1', point.y);
                }
            } else {
                node = this.createVmlNode('line');
                node.style.width = config.r * 2 + 'px';
                node.style.height = config.r * 2 + 'px';
                node.style.left = config.x - config.r + 'px';
                node.style.top = config.y - config.r + 'px';
    ...
    WorldMap.prototype = {
        ...
        createMarkers: function (markers) {
          ...
          defaultConfig = { latLng: [0, 0], r: 5, fill: 'white', stroke: '#505050' };
            ...
            var group2 = this.canvas.createGroup();
            ...
            for (i = 0; i < markers.length; i++) {
                ...
                for (j = i + 1; j < markers.length; j++) {
                    point2 = this.latLngToPoint.apply(this, markers[j]["latLng"]);
                    marker = this.canvas.createLine(markerConfig, point.x, point.y, point2.x, point2.y);
                    if (this.canvas.mode == 'svg') {
                        marker.setAttribute('class', 'jvectormap-marker');
                        marker.setAttribute('data-locl', markers[i]["local"]);
                        marker.setAttribute('data-locl2', markers[j]["local"]);
                    } else {
                        $(marker).addClass('jvectormap-marker').attr('data-locl', markers[i]["local"]).attr('data-locl2', markers[j]["local"]); ;
                    }
                    $(group2).append(marker);
                }

            }

            this.canvas.canvas.appendChild(group);
            this.canvas.canvas.appendChild(group2);
        },
        ...
        repositionMarkers: function () {
          ...
          point;
            ...
            //alert($(this.canvas.canvas.id + " svg g line[data-index2=37]").length);
            for (i = 0; i < this.markers.length; i++) {
                //alert(this.markers[i].config.local + ' = ' + this.markers[i].config.name);
                point = this.latLngToPoint.apply(this, this.markers[i].config.latLng);
                this.markers[i].element.setPosition(point);

                $.each($(this.canvas.canvas.id + " svg g line[data-locl=" + this.markers[i].config.local + "]"), function (index, theLine) {
                    $(theLine).attr('x1', point.x);
                    $(theLine).attr('y1', point.y);
                });

                $.each($(this.canvas.canvas.id + " svg g line[data-locl2=" + this.markers[i].config.local + "]"), function (index, theLine) {
                    $(theLine).attr('x2', point.x);
                    $(theLine).attr('y2', point.y);
                });
            }
        },
